...
def calc_histo(image, channels, bsize, range):
    shape = bsize if len(channels) > 1 else (bsize[0], 1)
    hist = cv2.calcHist([image], [1], None, bsize, ranges)
    gap = np.divide(range[1::2], bsize)
...
image = cv2.imread("images/hist_stretch.jpg", cv2.IMREAD_COLOR)   # 영상읽기
if image is None: raise Exception("영상 파일 읽기 오류")

bsize, ranges = [64, 64, 64], [0,256]                        # 계급 개수 및 화소 범위
hist = calc_histo(image, 3, bsize, ranges)
